...
const toplist = await oada.get({path: `/bookmarks/trellisfw/asns`}).then(r=>r.data);
const days = Object.keys(toplist['day-index']).filter(k => k.match(/[0-9]{4}-[0-9]{2}-[0-9]{2}/));
console.log(`Found these ${days.length} days in main list: `, days);

const asns = await Promise.each(days, async day => {
  if (!await askYN(`  ${day}: Proceed with retrieving ASNs for day ${day}?`)) {
    return; // see if they want to do the next day
  }
  const index = await oada.get({path: `/bookmarks/trellisfw/asns/day-index/${day}` }).then(r=>r.data);
  ...
  if (monitiskeys.length && await askYN(`  ${day}: Of those, ${monitiskeys.length} are monitis.  Delete them?`)) {
  ...
  await yesOrDie(`  ${day}: Have ${regularasnkeys.length} regular ASN keys: retrieve their jobs and hashes to look for success/failure and duplicates?`);
  ...
  await Promise.each(regularasnkeys, async key => {
    ...
    const joblist = await oada.get({ path: `/${resid}/_meta/services/target/jobs` }).then(r=>r.data);
    ...
    asninfo[resid] = { key, id: resid, jobs: {}, hash: h, looksLegit: !!(cleanasn._BEGIN || cleanasn.ZSHPMNT05) };
    const info = asninfo[resid];

    // Get all the job _refs
    const jobrefs = Object.keys(joblist).map(j => ({ key: j, id: joblist[j]._ref }));
    console.log(`  ${day}: Retrieving ${jobrefs.length} jobs for asn ${resid}`);
  ...
  console.log(`  ${day}: Have ${multijobs.length} asn's with multiple jobs listed:`, 
    multijobs.map(j => ({ 
      key: j.key, 
      id: j.id, 
      ...
      numberSuccessJobs: Object.keys(j.jobs).map(jk => j.jobs[jk]).filter(job => job.ISSUCCESS).length,
    }))
  );

  const timeouts = Object.keys(asninfo)
    .map(ak => asninfo[ak])
    .filter(a => Object.keys(a.jobs)
      .map(jk => a.jobs[jk])
      .filter(j => j.ISTIMEOUT).length > 1
    );

  console.log(`  ${day}: Have ${timeouts.length} timeout errors: `, 
    timeouts.map(j => ({
      key: j.key, 
      id: j.id, 
      looksLegit: j.looksLegit, 
  ...
  console.log(`  ${day}: Have ${errors.length} error jobs as the last job for an asn:`,
    errors.map(e => ({
      key: e.key, 
      id: e.id, 
