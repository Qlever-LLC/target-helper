...
describe('success job', () => {
  before(async function () {
    ...
    await Promise.each(doctypes, async (doctype) => {
      ...
      await con.put({
        path: `/resources/${i.key}/_meta/lookups/${i.name.singular}`,
        data: meta,
      });
  ...
  _.each(doctypes, (doctype) => {
    describe('#' + doctype, () => {
      ...
      it(
        'should link to the PDF at _meta/vdoc/pdf in the ' +
          doctype +
          ' resource',
        async () => {
          const result = await con
            .get({ path: `/resources/${i.key}/_meta/vdoc` })
            .then((r) => r.data);
      ...
      it(
        ...
        async () => {
          ...
          const result = await con
            .get({
              path: `/bookmarks/services/target/jobs-success/day-index/${day}/${j.key}`,
            })
            .then((r) => r.data);
