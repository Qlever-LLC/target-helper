  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const targetSuccess = async () => {
        ...
        trace(
          "Linking _ref's into pdf/_meta, job.result before refs is: ",
          job.result,
          ', and after refs replacement is ',
          recursiveReplaceLinksWithRefs(job.result)
        );
        await oada.put({
          path: `/${pdfid}/_meta`,
          data: {
        ...
        async function recursivePutVdocAtLinks(obj) {
          ...
          return Promise.each(_.keys(obj), (k) =>
            recursivePutVdocAtLinks(obj[k])
          );
        }
        ...
        async function recursiveSignLinks(obj) {
          if (!obj || typeof obj !== 'object') return;
          if (obj._id)
            return signResourceForTarget({ _id: obj._id, oada, log });
        ...
        await Promise.each(_.keys(versionedResult), async (doctype) => {
        ...
        await Promise.each(_.keys(result), async (doctype) => {
          ...
          await Promise.each(_.keys(doclist), async (dockey) => {
            ...
            trace(
              `Fetching lookups for doctype = ${doctype}, doc = `,
              doc,
              `, getting /${doc._id}/_meta/lookups`
            );
            const lookups = await oada
              .get({ path: `/${doc._id}/_meta/lookups` })
              .then((r) => r.data);
            ...
            switch (doctype) {
              ...
              case 'fsqa-certificates':
                ...
                break;
              case 'cois':
                const holder = await oada
                  .get({ path: `/${lookups.coi.holder._ref}` })
                  .then((r) => r.data);
                trace(`Retrieved holder `, holder);
                await Promise.each(
                  _.keys(holder['trading-partners']),
                  async (tplinkkey) => {
          ...
          await Promise.each(shares, async (s) => {
            ...
            const user = await oada
              .get({ path: `/${tp._id}/user` })
              .then((r) => r.data);
            ...
            if (tpkey.match(/REDDYRAW/)) {
              ...
              trace(
                'pdf is only generated for fsqa-audits or cois, doctype is ',
                doctype
              );
              mask = {
            ...
            const resid = await oada
              .post({
                path: `/resources`,
                headers: {
                  'content-type': tree.bookmarks.services['*'].jobs['*']._type,
                },
                data: {
        ...
        try {
          ...
          await Promise.each(_.keys(c.body.updates), async (k) => {
            ...
            if (v.status && v.status === 'error') {
              ...
              await unwatch();
              if (v.information)
                error(`Target job [${jobId}] failed: ${v.information}`);
              throw new Error(
                'Target returned error: ' + JSON.stringify(update, false, '  ')
              );
            }
          });
        } catch (e) {
      ...
      const watch = async () => {
        ...
        return await oada
          .get({ path: `/bookmarks/services/target/jobs/${jobId}` })
          .then((r) => r.data);
      };
...
async function pushSharesForFacility({ facilityid, doc, dockey, shares }) {
  ...
  _.each(_.keys(ei), (tpkey) => {
    ...
    if (
      !_.find(
        ei[tpkey].facilities,
        (flink, fmasterid) => flink._id === facilityid
      )
    )
...
async function signResourceForTarget({ _id, oada, log }) {
  ...
  try {
    ...
    async function hasTranscriptionSignature(res) {
      ...
      trace(
        `#signResourceForTarget: Checked for signature, got back trusted ${trusted} valid ${valid} unchanged ${unchanged} payload ${payload}`
      );
    ...
    trace(
      '#signResourceForTarget: Did not find existing ' +
        signatureType +
        ' signature, signing...'
    );
...
function treeForDocType(doctype) {
  ...
  if (singularType.match(/s$/)) singularType = singularType.replace(/s$/, '');
    ...
    if (config.get('tradingPartnersEnabled')) {
      await Promise.each(['601af61b53b391000e4a7a3e'], async (tp) => {
        ...
        if (tp_doc_exists !== 200) {
          info(
            `/bookmarks/trellisfw/trading-partners/${tp}/shared/trellisfw/documents does not exist, creating....`
          );
